...
-- Standalone echasnovski/mini.pick is gone: mini.nvim bundles mini.pick, so
-- require('mini.pick') always resolved to the bundled copy and the separate
-- install was pure dead weight.
...
local function map(lhs, wincmd, dir, desc)
  ...
  -- Terminal mode: leave terminal mode first (via feedkeys, since vim.cmd
  -- only takes Ex commands), then navigate once back in Normal mode.
  vim.keymap.set("t", lhs, function()
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<C-\\><C-n>", true, false, true), "n", false)
    vim.schedule(function()
      nav(wincmd, dir)
    end)
  end, { silent = true, noremap = true, desc = desc .. " (term)" })
end
